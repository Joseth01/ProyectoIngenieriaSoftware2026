  ...
  <ion-page>
    <ion-content :fullscreen="true" class="page-bg">
      ...
      <div class="body-pad">
        ...
        <div class="sec-title" style="margin-top:20px;margin-bottom:10px">
        ...
        <template v-for="r in tiposReporte" :key="r.key">
          ...
          >
            ...
            <span class="chev" :class="{ 'chev-open': reporteActivo === r.key }">
              ›
            </span>
          </div>
          ...
          <div v-if="reporteActivo === r.key" class="rpt-panel">
                ...
                <table class="rpt-tbl">
                  ...
                  <tbody>
                    <tr v-for="m in datosRendimiento" :key="m.mes">
                      ...
                      <td :class="getDeltaClass(m.delta)">
                        {{ formatDelta(m.delta) }}
                      </td>
                    </tr>
                  </tbody>
            ...
            <template v-else-if="r.key === 'salud'">
              <div class="rpt-stats">
                <div class="rs-card">
                  <div class="rs-val">
                    {{ datosSalud.filter(a => !a.sinPeso).length }}
                  </div>
              ...
              <div class="tbl-wrap">
                <table class="rpt-tbl">
                  ...
                  <tbody>
                    <tr
                      v-for="a in datosSalud"
                      :key="a.arete"
                      :class="{ 'row-warn': a.sinPeso }"
                    >
                      <td>{{ a.arete }}</td>
                      <td>{{ a.nombre }}</td>
                      <td>{{ a.ultimoPeso }}</td>
...
// ── Helpers de pesajes por animal ───────────────────────────────────────────
// getAnimales() no siempre trae animal.pesajes.
// Por eso cruzamos la lista de animales con getPesajes() usando animal_id.
function tiempoFecha(valor: string | null | undefined): number {
  if (!valor) {
    return 0;
  }
...
function ordenarPesajesDesc(a: PesajeDto, b: PesajeDto): number {
  const fechaA = tiempoFecha(a.fecha);
  const fechaB = tiempoFecha(b.fecha);

  if (fechaA !== fechaB) {
    return fechaB - fechaA;
  }

  const creadoA = tiempoFecha(a.created_at);
  const creadoB = tiempoFecha(b.created_at);
...
const pesajesPorAnimal = computed(() => {
  const mapa = new Map<number, PesajeDto[]>();

  for (const pesaje of pesajes.value) {
    ...
    if (!mapa.has(animalId)) {
      mapa.set(animalId, []);
    }

    mapa.get(animalId)!.push(pesaje);
  }

  for (const lista of mapa.values()) {
    lista.sort(ordenarPesajesDesc);
  }

  return mapa;
});

function obtenerPesajesAnimal(animal: AnimalDto): PesajeDto[] {
  return pesajesPorAnimal.value.get(Number(animal.id)) || [];
}

function obtenerUltimoPesajeAnimal(animal: AnimalDto): PesajeDto | null {
  const lista = obtenerPesajesAnimal(animal);
...
const barras = computed(() => {
  ...
  return Array.from({ length: 6 }, (_, i) => {
    ...
    const grupo = pesajes.value.filter((p) =>
      p.fecha?.startsWith(key)
    );
    ...
    return {
      ...b,
      pct: Math.round((b.prom / max) * 90) || 6,
    };
  });
});
...
const datosInventario = computed(() => {
  ...
  for (const animal of animales.value) {
    const raza = animal.raza?.nombre || 'Sin raza';
    ...
    const entrada = map.get(raza)!;
    entrada.count++;

    const ultimo = obtenerUltimoPesajeAnimal(animal);

    if (ultimo) {
      entrada.pesoTotal += pesoNumerico(ultimo);
      entrada.conPeso++;
    }
  }

  const totalAnimales = animales.value.length || 1;

  return Array.from(map.entries())
    .map(([raza, datos]) => ({
      raza,
      cantidad: datos.count,
      pct: ((datos.count / totalAnimales) * 100).toFixed(1),
      promedio: datos.conPeso > 0
        ? (datos.pesoTotal / datos.conPeso).toFixed(0)
        : '—',
    }))
...
const datosSalud = computed(() =>
  animales.value.map((animal) => {
    const ultimo = obtenerUltimoPesajeAnimal(animal);

    return {
      arete: animal.numero_arete,
      nombre: animal.nombre || '—',
      finca: animal.finca?.nombre || 'Sin finca',
      ultimoPeso: ultimo
        ? `${pesoNumerico(ultimo).toFixed(0)} kg`
        : '— kg',
      fechaUltimo: ultimo
...
const datosFincas = computed(() => {
  ...
  for (const animal of animales.value) {
    const finca = animal.finca?.nombre || 'Sin finca asignada';
  ...
  return Array.from(map.entries()).map(([finca, datos]) => ({
    finca,
    animales: datos.animales,
    promedio: datos.conPeso > 0
      ? (datos.pesoTotal / datos.conPeso).toFixed(0)
      : '—',
    total: datos.pesoTotal.toFixed(0),
  }));
});
...
const tiposReporte = [
  {
    ...
    sub: 'Eficiencia de engorde y conversión',
  },
  {
    key: 'inventario',
    ico: '📋',
    titulo: 'Inventario de Ganado',
    sub: 'Resumen por razas y edades',
  },
  {
    key: 'salud',
    ico: '🏥',
    titulo: 'Salud y Condición',
    sub: 'Estado de peso por animal',
  },
  {
    key: 'fincas',
    ico: '🗺️',
    titulo: 'Distribución por Fincas',
    sub: 'Densidad de carga por sector',
  },
];
...
const seleccionarReporte = async (tipo: string) => {
  ...
  try {
    ...
    // Silencioso para no interrumpir la consulta del reporte.
  }
};
...
function buildPDF(
  ...
  const addFooter = () => {
    ...
    doc.setFontSize(7);
    doc.text(
      'BovWeight CR — Sistema de gestión ganadera',
      ML,
      H - 5
    );
    ...
    sec.filas.forEach((fila, ri) => {
      ...
      fila.forEach((cell, i) => {
        const maxChars = Math.max(
          5,
          Math.floor(sec.widths[i] / 2.4)
        );

        doc.text(
          trunc(String(cell), maxChars),
          cx + 2,
          y
        );
...
const descargarPDFReporte = async (tipo: string) => {
  ...
  if (tipo === 'rendimiento') {
    ...
    doc = buildPDF(
      ...
      `Total: ${animales.value.length} cabezas — ${fecha}`,
      [
        {
...
const descargarPDFCompleto = async () => {
  ...
  const doc = buildPDF(
    'Reporte General',
    `Informe completo del hato — ${fecha}`,
    [
      {
...
.rpt-tbl tbody tr:nth-child(even) {
  background: #F9FAF9;
}
